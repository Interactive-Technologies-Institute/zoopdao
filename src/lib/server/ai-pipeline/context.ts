...
import { getOrUpdateRollingSummary } from '$lib/server/ai-pipeline/summary';
...
export type Round7Context = {
	...
	currentUserProfile: PlayerContext;
	proposalTitle: string | null;
...
function personaNameFromAgentKey(agentKey: string): string | null {
	switch (agentKey) {
		case 'galeria':
			return 'Galeria';
		case 'tuga':
...
async function fetchProposalContext(
	...
	if (!proposalId) {
		return { proposalTitle: null, proposalPointText: null, proposalContextText: null, proposalFullText: null };
	}
	...
	const proposalTitle = (proposal.title ?? '').trim() || null;
	...
	return { proposalTitle, proposalPointText, proposalContextText, proposalFullText };
}
...
function mapRowsToChatHistory(
	...
	return rows.map((row) => {
		...
		if (senderType === 'human') {
			...
			const agentId = typeof row?.metadata?.agent_id === 'string' ? (row.metadata.agent_id as string) : '';
			const agentKey = agentId.startsWith('ai-agent-') ? agentId.slice('ai-agent-'.length) : '';
			const personaName = agentKey ? personaNameFromAgentKey(agentKey) : null;
			senderName = personaName ?? (row.agent_role ? toTitleCaseRole(row.agent_role) : 'AI Agent');
		}
		return {
...
export async function buildRound7Context(params: {
	...
	const currentUserProfile = await fetchPlayerContext(supabaseAdmin, gameId, userId);
